...
import 'package:kivicare_flutter/screens/shimmer/screen/product_list_shimmer_screen.dart';
import 'package:kivicare_flutter/screens/woocommerce/component/product_component.dart';
import 'package:kivicare_flutter/screens/woocommerce/component/sort_filter_bottom_sheet_component.dart';
...
class ProductListScreen extends StatefulWidget {
  ...
  const ProductListScreen({this.categoryId, this.refreshCall, this.categoryName});
...
class _ProductListScreenState extends State<ProductListScreen> {
  ...
  Future<void> removeFromFav({required int productId, bool isWishListed = true}) async {
    setState(() {
      isWishListed = !isWishListed;
    });
    await removeFromWishlist(productId: productId).then((value) {
      toast(value.message);
      init();
    }).catchError((e) {
      toast(e.toString());
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: appBarWidget(
        widget.categoryName != null ? widget.categoryName.validate() : locale.products,
            ...
            children: [
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  child: Text('${getIntAsync('${CartKeys.cartItemCountKey} of ${userStore.userName}')}', style: secondaryTextStyle(color: Colors.white, size: 12)),
                ),
              ).visible(getIntAsync('${CartKeys.cartItemCountKey} of ${userStore.userName}') > 0)
            ],
          )
        ],
      ),
