...
class _OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  Future<void> onDeleteOrder() async {
    showConfirmDialogCustom(
      ...
      onAccept: (c) async {
        appStore.setLoading(true);
        await deleteOrder(orderId: widget.orderId.validate()).then((value) {
        ...
        return CancelOrderFormComponent(
          ...
          callback: (text) {
            showConfirmDialogCustom(
              ...
              onAccept: (c) async {
                appStore.setLoading(true);
                await cancelOrder(orderId: widget.orderId.validate(), note: text).then((value) {
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          if (orderDetails.couponLines.validate().isNotEmpty)
                            Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(locale.appliedCoupons, style: secondaryTextStyle()),
                                8.height,
                                ...orderDetails.couponLines
                                    .validate()
                                    .map(
                                      (e) => Row(
                                        children: [
                                          Text(locale.code.suffixText(value: ' - '), style: secondaryTextStyle()),
                                          4.width,
                                          Text(e.code.validate(), style: secondaryTextStyle()),
                                          24.width,
                                          PriceWidget(price: e.discount.validate(), textStyle: secondaryTextStyle()).expand(),
                                        ],
                                      ),
                                    )
                                    .toList(),
