...
class _CartScreenState extends State<CartScreen> {
  ...
  Future<void> onAcceptRemoveItem(CartItemModel cartItem) async {
    appStore.setLoading(true);
    await removeCartItem(productKey: cartItem.key.validate()).then((value) {
      init();
    }).catchError((e) {
      toast(e.toString(), print: true);
      appStore.setLoading(false);
                                      ...
                                      Row(
                                        children: [
                                          ...
                                          Icon(
                                            ...
                                            size: 20,
                                          ).paddingSymmetric(horizontal: 12, vertical: 6).onTap(() async {
                                            if (cartItem.quantity.validate() > 1) {
                                              int originalQua = cartItem.quantity.validate();
                                              cartItem.quantity = cartItem.quantity.validate() - 1;
                                              appStore.setLoading(true);
                                              await updateCartItem(productKey: cartItem.key.validate(), quantity: cartItem.quantity.validate()).whenComplete(() {
                                                init();
                                              }).catchError((e) {
                                                toast(e.toString());
                                                cartItem.quantity = originalQua;
                                                setState(() {});
                                                appStore.setLoading(false);
                                              });
                                            }
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              8.height,
                              if (double.tryParse(cart!.totals!.totalShipping.validate()) != null && double.parse(cart!.totals!.totalShipping.validate()) > 0)
                                Row(
                                  children: [
                  ...
                  onTap: () async {
                    if (!appStore.isLoading)
                      await Future.forEach<CartItemModel>(cart!.items.validate(), (element) async {
                        ...
                        CheckoutScreen(
                          cartDetails: cart!,
                          itermRemoved: () => init(showLoader: false),
                        ).launch(context);
                      }).catchError((e) {
                        appStore.setLoading(false);
