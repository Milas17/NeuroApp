...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Future<void> init() async {
    ...
    future = await getPaymentMethods().then((value) {
  ...
  Future<void> removeCartItemCoupon() async {
    ...
    await Future.forEach<CartCouponModel>(
      cart.coupons.validate(),
      (element) {
        removeCoupon(code: element.code.validate()).then((value) {
        ...
        children: [
          AnimatedScrollView(
            ...
            children: [
              ...
              if (!isSameAddress || shippingAddress == null) Text('${locale.lblPostalCode}: ${billingAddress.postcode}', style: secondaryTextStyle()).visible(billingAddress.postcode.validate().isNotEmpty),
              if (!isSameAddress || shippingAddress == null) Text('${locale.lblContactNumber}: ${billingAddress.phone}', style: secondaryTextStyle()).visible(billingAddress.phone.validate().isNotEmpty),
              ...
              ...[
                if (paymentMethods.isEmpty && isPaymentGatewayLoading)
                  Offstage()
                else if (!isPaymentGatewayLoading) ...[
                  if (paymentMethods.isEmpty)
                    ...
                    Container(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (ctx, index) {
                          if (paymentMethods[index].id != 'cod')
                            return Offstage();
                          else
                            return Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Row(
                                  children: [
                                    Icon(selectedPaymentMethod == paymentMethods[index] ? Icons.radio_button_checked : Icons.radio_button_off, color: context.primaryColor, size: 20),
                                    8.width,
                                    Text(paymentMethods[index].title.toString().validate(), style: primaryTextStyle()),
                                  ],
                                ),
                                8.height,
                                Container(
                                  child: Text(paymentMethods[index].description.validate(), style: secondaryTextStyle(size: 12)),
                                  decoration: BoxDecoration(color: context.scaffoldBackgroundColor, borderRadius: radius()),
                                  padding: EdgeInsets.all(8),
                                ).visible(selectedPaymentMethod == paymentMethods[index])
                              ],
                            ).onTap(
                              () {
          ...
          Observer(builder: (_) => LoaderWidget().center().visible(appStore.isLoading || isPaymentGatewayLoading)),
        ],
      ),
    );
  }
