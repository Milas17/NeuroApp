    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              ...
              InkWell(
                ...
                onTap: () {
                  toast(locale.lblCopiedToClipboard);
          ...
          8.height,
          if (amount != null)
            if (amount.validate() < coupon.minimumAmount.toDouble() && amount.validate() < coupon.maximumAmount.toDouble())
              Text(
                'Unlock Savings with a Minimum Spend!',
                style: secondaryTextStyle(color: Colors.grey, fontStyle: FontStyle.italic),
              )
            else if (amount.validate() > coupon.maximumAmount.toDouble())
              Text(
                'Your cart value exceeds the maximum spend limit.',
                style: secondaryTextStyle(color: Colors.grey, fontStyle: FontStyle.italic),
              )
            else
              Text(
                "Coupon is applicable ! 🚀 You're in the Savings Zone!",
                style: primaryTextStyle(color: ratingBarLightGreenColor, fontStyle: FontStyle.italic),
