...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> checkIsAppInReview() async {
    await setupFirebaseRemoteConfig().then((remoteConfig) async {
      if (isIOS) {
        await setValue(AppKeys.hasInReviewKey, remoteConfig.getBool(AppKeys.hasInReviewAppStore), print: true);
      } else if (isAndroid) {
        await setValue(AppKeys.hasInReviewKey, remoteConfig.getBool(AppKeys.hasInReviewPlayStore), print: true);
      }
    }).catchError((e) {
      toast(e.toString());
    });
