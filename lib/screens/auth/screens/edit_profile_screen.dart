...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  final SignatureController _controller = SignatureController(
    ...
    exportBackgroundColor: signatureBackgroundColor.withValues(alpha: 0.2),
  );
  ...
  void setUserData(UserModel userData) {
    ...
    if (!userData.bloodGroup.isEmptyOrNull) bloodGroup = bloodGroupList.where((element) => element == userData.bloodGroup.validate(value: '')).first;
    if (isDoctor()) {
  ...
  Future<void> saveUserDetails() async {
    ...
    await updateProfileAPI(data: request, profileImage: selectedProfileImage, doctorSignature: doctorSignature).then((value) {
      appStore.setLoading(false);
      if (value != null) setUserData(value);
    }).catchError((e) {
      appStore.setLoading(false);
      toast(e.toString());
    });
  ...
  Future<void> _handleCurrentLocationClick() async {
    ...
    try {
      ...
      final address = await getUserLocation();
      addressCont.clear();
