...
class _Step3FinalSelectionScreenState extends State<Step3FinalSelectionScreen> {
  ...
  void selectServices() async {
    ...
    ).launch(context, pageRouteAnimation: pageAnimation, duration: pageAnimationDuration).then((value) => LiveStream().emit(CHANGE_DATE, true));
      ...
      body: Form(
        ...
        child: AnimatedScrollView(
          ...
          children: [
            ...
            Row(
              children: [
                if (isProEnabled()) SelectedClinicComponent().expand(),
            ...
            Observer(builder: (context) {
              if (multiSelectStore.selectedServiceIds.isNotEmpty) {
                return AppointmentSlots(
                  date: isUpdate ? widget.data?.getAppointmentSaveDate : getAppointmentDate,
                  clinicId: isUpdate ? widget.data?.clinicId.validate() : appointmentAppStore.mClinicSelected!.id,
                  doctorId: isUpdate ? widget.data?.doctorId.validate() : appointmentAppStore.mDoctorSelected!.iD.validate().toString(),
                  serviceId: multiSelectStore.selectedServiceIds,
