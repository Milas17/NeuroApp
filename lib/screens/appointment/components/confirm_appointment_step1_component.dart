...
class _ConfirmAppointmentStep1ComponentState extends State<ConfirmAppointmentStep1Component> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        42.height,
        Text("${isDoctor() ? '${locale.lblDr}${userStore.firstName}' : '${userStore.firstName}'}, ${locale.lblAppointmentConfirmation}.", style: primaryTextStyle(), textAlign: TextAlign.center),
        16.height,
        Row(
        ...
        Row(
          ...
          children: [
            ...
            if (isDoctor() || isReceptionist())
              ...
              Text(
                "${appointmentAppStore.mDoctorSelected?.displayName.validate().split(' ').first.capitalizeEachWord().prefixText(value: '${locale.lblDr}')}",
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<int>(
                    ...
                    onChanged: (value) {
                      ...
                      if (appStore.paymentMode == PAYMENT_OFFLINE) {
                        appointmentAppStore.setBookingStatus(BookedStatusInt);
                      } else {
                        appointmentAppStore.setBookingStatus(PendingStatusInt);
                      }
                    },
                  ),
                ),
