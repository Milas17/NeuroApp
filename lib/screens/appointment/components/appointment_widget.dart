...
class _AppointmentWidgetState extends State<AppointmentWidget> {
  ...
  bool get showGoogleMeet {
    ...
    if (!isReceptionist()) {
      if (widget.upcomingData.googleMeetData.validate().isNotEmpty && (widget.upcomingData.status.toInt() != CancelledStatusInt || widget.upcomingData.status.toInt() != PendingStatusInt) && widget.upcomingData.status.toInt() != CheckOutStatusInt) {
  ...
  bool get showZoom {
    ...
    if (!isReceptionist()) {
      if (widget.upcomingData.zoomData != null && (widget.upcomingData.status.toInt() != CancelledStatusInt || widget.upcomingData.status.toInt() != PendingStatusInt) && widget.upcomingData.status.toInt() != CheckOutStatusInt) {
  ...
  void _handleZoomButton() {
    if ((isDoctor() || isPatient()) && widget.upcomingData.zoomData != null) {
      zoomLaunch(isPatient() ? widget.upcomingData.zoomData!.joinUrl.validate() : widget.upcomingData.zoomData!.startUrl.validate());
    } else {
      toast(locale.lblYouCannotStart);
    }
  }
    ...
    await showInDialog(
      ...
      builder: (p0) {
        return AddReviewDialog(
          customerReview: widget.upcomingData.doctorRating,
          doctorId: widget.upcomingData.doctorId.validate().toInt(),
        );
      },
    ).then((value) {
      ...
      children: [
        ...
        Column(
          ...
          children: [
            ...
            1.height,
            if (isReceptionist()) Text(widget.upcomingData.doctorName.validate().prefixText(value: "${locale.lblDr}"), style: boldTextStyle(size: 12)),
          ...
          Row(
            children: [
              ...
              commonAppButton(
                ...
                color: appStore.isDarkModeOn ? context.iconColor.withValues(alpha: 0.2) : context.iconColor,
                topLeft: (showViewButton || showGoogleMeet || showZoom) ? 0 : defaultRadius,
                bottomLeft: (showViewButton || showGoogleMeet || showZoom) ? 0 : defaultRadius,
                topRight: (showCheckInButton || showReviewButton || showZoom) ? 0 : defaultRadius,
                bottomRight: (showCheckInButton || showReviewButton || showZoom) ? 0 : defaultRadius,
              ).visible(showEncounterButton),
              commonAppButton(
                buttonText: isPatient() ? locale.lblJoin : locale.lblStart,
                onTap: _handleZoomButton,
                color: zoomColor,
                topLeft: (showViewButton || showEncounterButton) ? 0 : defaultRadius,
                bottomLeft: (showViewButton || showEncounterButton) ? 0 : defaultRadius,
                ...
                child: TextIcon(
                  text: isDoctor() ? locale.lblStart : locale.lblJoin,
                  suffix: ic_telemed.iconImage(size: 18, color: Colors.white),
                  textStyle: boldTextStyle(size: 12, color: Colors.white),
                ),
              ).visible(showZoom),
              commonAppButton(
          ...
          children: [
            ...
            /*Positioned(
              ...
              child: Container(
                ...
                decoration: boxDecorationDefault(color: primaryColor.withValues(alpha: 0.2), shape: BoxShape.circle),
                child: ic_telemed.iconImage(color: primaryColor),
              ),
            )*/
          ],
        ),
