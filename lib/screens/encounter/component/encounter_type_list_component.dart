...
import '../../doctor/screens/add_prescription_screen.dart';
...
class _EncounterTypeListState extends State<EncounterTypeList> {
  ...
  Widget buildEncounterPrescriptionList() {
    if (widget.encounterData.prescription.validate().isEmpty) return NoDataWidget(title: locale.lblNoPrescriptionFound.capitalizeFirstLetter()).paddingBottom(16);
    return AnimatedWrap(
      spacing: 16,
      runSpacing: 12,
      ...
      itemBuilder: (context, index) {
        ...
        return EncounterTypeComponent(
          ...
          onTap: () {
            ...
            switch (widget.encounterType) {
              case PROBLEM:
                deleteTitle = locale.lblDoYouWantToDeleteProblem;
                break;
              case OBSERVATION:
                deleteTitle = locale.lblDoYouWantToDeleteObservation;
                break;
              case NOTE:
                deleteTitle = locale.lblDoYouWantToDeleteNote;
                break;
              default:
                deleteTitle = '';
            }
            showConfirmDialogCustom(
              context,
              title: deleteTitle.capitalizeFirstLetter(),
              dialogType: DialogType.DELETE,
              onAccept: (p0) {
