...
class _BillDetailsScreenState extends State<BillDetailsScreen> {
  ...
  Future<void> getTaxDataApi() async {
    if (isProEnabled()) {
      appStore.setLoading(true);
      ...
      request.putIfAbsent(ConstantKeys.visitTypeKey, () => selectedServiceRequest);
      await getTaxData(request).then((data) {
        taxData = data;
        setState(() {});
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (billDetail != null && billDetail!.taxes.validate().isNotEmpty) Divider(height: 8, color: context.dividerColor),
                        if (billDetail != null && billDetail!.taxes.validate().isNotEmpty)
                          Row(
                            children: [
                        ...
                        if (billDetail != null && billDetail!.taxes.validate().isNotEmpty) 6.height,
                        if (billDetail != null && billDetail!.taxes.validate().isNotEmpty)
                          ...billDetail!.taxes.validate().map<Widget>((e) {
                            return Row(
                              children: [
  ...
  Widget patientDetails({required PatientBillModule patientBillData}) {
    return Column(
      ...
      children: [
        RichTextWidget(
          list: [
            TextSpan(text: locale.lblPatientName + ': ', style: boldTextStyle(size: 14)),
            TextSpan(text: '${patientBillData.patient!.displayName.validate()}', style: primaryTextStyle(size: 14)),
          ],
        ),
        6.height,
        RichTextWidget(
          list: [
            TextSpan(text: locale.lblGender2 + ': ', style: boldTextStyle(size: 14)),
            TextSpan(text: '${patientBillData.patient!.gender.validate().capitalizeFirstLetter()}', style: primaryTextStyle(size: 14)),
          ],
        ),
        6.height,
        RichTextWidget(
          list: [TextSpan(text: locale.lblDOB + ': ', style: boldTextStyle(size: 12)), TextSpan(text: '${patientBillData.patient?.dob.validate()}', style: primaryTextStyle(size: 12))],
        ),
      ],
    );
  }
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Text(locale.lblServiceName, style: boldTextStyle(size: 12), textAlign: TextAlign.start).fit(fit: BoxFit.none).expand(flex: 2),
              ...
              itemBuilder: (context, index) {
                ...
                return Padding(
                  ...
                  child: Row(children: [
                    ...
                    Text('${appStore.currencyPrefix.validate(value: '')}${data.price.validate()}${appStore.currencyPostfix.validate(value: '')}', style: primaryTextStyle(size: 12), textAlign: TextAlign.center).expand(),
                    Text('${data.qty.validate()}', style: primaryTextStyle(size: 12), textAlign: TextAlign.center),
                    Text('${appStore.currencyPrefix.validate(value: '')}$total${appStore.currencyPostfix.validate(value: '')}', style: primaryTextStyle(size: 12), textAlign: TextAlign.end).expand(flex: 1),
                  ]),
                );
              },
