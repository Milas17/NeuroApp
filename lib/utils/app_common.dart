...
String getRoleWiseName({required String name}) {
  if (isDoctor()) {
    return '${name.prefixText(value: '${locale.lblDr}')}';
  } else {
    return name;
  }
}
...
String getRoleWiseAppointmentFirstText(UpcomingAppointmentModel upcomingData) {
  if (isDoctor() || isReceptionist())
    ...
    return upcomingData.doctorName.validate().prefixText(value: '${locale.lblDr}');
}

SystemUiOverlayStyle defaultSystemUiOverlayStyle(BuildContext context, {Color? color, Brightness? statusBarIconBrightness}) {
  return SystemUiOverlayStyle(statusBarColor: color ?? context.primaryColor.withValues(alpha: 0.02), statusBarIconBrightness: statusBarIconBrightness ?? Brightness.light);
}
...
dynamic shimmerBoxInputDecoration({double? borderRadiusValue, Color? color, BoxShape? shape}) {
  return boxDecorationDefault(
    boxShadow: [],
    shape: shape,
    color: color ?? primaryColor.withValues(alpha: 0.2),
...
Future<void> dateBottomSheet(BuildContext context, {DateTime? bDate, Function(DateTime?)? onBirthDateSelected}) async {
  hideKeyboard(context);
  if (bDate == null) {
    bDate = DateTime.now();
  }
