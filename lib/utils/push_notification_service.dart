...
class PushNotificationService {
  ...
  Future<void> subScribeToTopic() async {
    if (isDoctor()) {
      await FirebaseMessaging.instance.subscribeToTopic("${FirebaseMsgConst.doctorWithUnderscoreKey}${userStore.userId}").then((value) {
        log("${FirebaseMsgConst.topicSubscribed}${FirebaseMsgConst.doctorWithUnderscoreKey}${userStore.userId}");
      });
    } else if (isReceptionist()) {
      await FirebaseMessaging.instance.subscribeToTopic("${FirebaseMsgConst.receptionistWithUnderscoreKey}${userStore.userId}").then((value) {
        log("${FirebaseMsgConst.topicSubscribed}${FirebaseMsgConst.receptionistWithUnderscoreKey}${userStore.userId}");
      });
    } else {
      await FirebaseMessaging.instance.subscribeToTopic("${FirebaseMsgConst.patientWithUnderscoreKey}${userStore.userId}").then((value) {
  ...
  Future<void> unsubscribeFirebaseTopic() async {
    if (isDoctor()) {
      await FirebaseMessaging.instance.unsubscribeFromTopic("${FirebaseMsgConst.doctorWithUnderscoreKey}${userStore.userId}").then((value) {
        log("${FirebaseMsgConst.topicUnSubscribed}${FirebaseMsgConst.doctorWithUnderscoreKey}${userStore.userId}");
      });
    } else if (isReceptionist()) {
      await FirebaseMessaging.instance.unsubscribeFromTopic("${FirebaseMsgConst.receptionistWithUnderscoreKey}${userStore.userId}").then((value) {
        log("${FirebaseMsgConst.topicUnSubscribed}${FirebaseMsgConst.receptionistWithUnderscoreKey}${userStore.userId}");
      });
    } else {
      await FirebaseMessaging.instance.unsubscribeFromTopic("${FirebaseMsgConst.patientWithUnderscoreKey}${userStore.userId}").then((value) {
