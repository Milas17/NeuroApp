...
Future<List<UpcomingAppointmentModel>> getPatientAppointmentList(
  ...
  setValue(
    SharedPreferenceKey.cachedAppointmentListKey,
    jsonEncode(res.upcomingAppointmentData.validate().map((e) => e.toJson()).toList()),
    print: true,
  );
...
Future<List<UpcomingAppointmentModel>> getReceptionistAppointmentList({
  ...
  AppointmentModel res = AppointmentModel.fromJson(await handleResponse(await buildHttpResponse(getEndPoint(
    endPoint: ApiEndPoints.getAppointmentEndPoint,
    params: param,
  ))));
  ...
  setValue(
    SharedPreferenceKey.cachedAppointmentListKey,
    jsonEncode(res.appointmentData.validate().map((e) => e.toJson()).toList()),
    print: true,
  );
  return appointmentList;
}
...
Future<List<List<AppointmentSlotModel>>> getAppointmentTimeSlotList({
  ...
  if (serviceId!.isNotEmpty) params.add('${ConstantKeys.serviceIdKey}=$serviceId');
